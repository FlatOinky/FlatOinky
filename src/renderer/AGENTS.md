...
- **Anchors** — `main.ts` tags game `<td>` cells with
  ...
  UI to game regions.
- **Game `Map` class** — FlatMMO declares `class Map` in `refs/js/maps.js`. A
  `transpileScript` pass rewrites `class`/`new`/`instanceof Map` to `GameMap`
  before injection, so the game no longer shadows the built-in `Map`. Renderer
  code can use `Map` as usual.

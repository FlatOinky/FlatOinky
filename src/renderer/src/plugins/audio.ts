import type { ClientPlugins, Lifecycle, Logger } from '../../client';
import { createProfileStorage, createSystemCollection } from '../client_storage';
...
export const initAudioSystem = async (
	...
	const storage = await createProfileStorage('systems', 'audio');
	const settings = storage.reactive('settings', initialAudioSettings);
	const registry = await createAudioRegistry(
		createSystemCollection<AudioPlay>('profile', 'audio/plays'),
	);
	const engine = createAudioEngine(lifecycle, settings);
	const sync = createAudioSync(settings, createLogger('Audio'));
	...
	const showWindow = () => {
		...
		audioWindow ??= initAudioWindow(lifecycle, { ui, storage }, registry, deps, closeWindow);
		paints.add(audioWindow.schedule);
		audioWindow.show();
	};

	const tray = mountAudioTray(lifecycle, ui, registry, deps, showWindow);
	...
	const audioMenu = settingsMenu.mountSection('Audio', [
		globalControls,
	...
	lifecycle.onCleanup(audioMenu.remove);

	plugins.registerCallbacks(lifecycle, {
		events: {
			startup: () => {
				hideUpstreamAudioControls(lifecycle);
				sync.reconcile();
